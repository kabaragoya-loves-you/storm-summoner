...
static int cmd_info(int argc, char **argv) {
  ...
  uint8_t cc = expression_get_cc_number();
  int16_t min, max;
  ...
  ESP_LOGI(TAG, "CC number: %u", (unsigned)cc);
  ESP_LOGI(TAG, "Connected: %s", connected ? "yes" : "no");
  ESP_LOGI(TAG, "Range: %d - %d", min, max);
  ESP_LOGI(TAG, "========================");
...
static int cmd_cc(int argc, char **argv) {
  ...
  int cc = cc_args.cc_num->ival[0];
  if (cc < 0 || cc > 127) {
    ESP_LOGE(TAG, "CC number must be 0-127");
    return 1;
  }
  
  expression_set_cc_number((uint8_t)cc);
  ESP_LOGI(TAG, "Expression CC number set to %d", cc);
...
esp_err_t expression_console_init(void) {
  ...
  // cc command
  cc_args.cc_num = arg_int1(NULL, NULL, "<0-127>", "CC number");
  cc_args.end = arg_end(2);
  
  const esp_console_cmd_t cc_cmd = {
    .command = "cc",
    .help = "Set MIDI CC number",
    .hint = NULL,
    .func = &cmd_cc,
    .argtable = &cc_args
  };
  esp_console_cmd_register(&cc_cmd);
