...
static void apply_tempo_nudge(uint8_t midi_value, scene_t* scene) {
  ...
  tempo_nudge_direction_t dir =
    (tempo_nudge_direction_t)scene_get_expression_tempo_nudge_direction(scene_get_current_index());
  float scale;
...
static void handle_expression_value(const event_t* event, void* context) {
  ...
  if (mapping->output_type == OUTPUT_TYPE_TEMPO_NUDGE) {
    ...
    uint8_t nudge_value = smart_filter_process(&s_expression_filter, raw_value, &value_changed);
...
static void handle_sustain_event(const event_t* event, void* context) {
  ...
  scene_notify_hold_for(&scene->sustain, pressed);
...
static void handle_sostenuto_event(const event_t* event, void* context) {
  ...
  scene_notify_hold_for(&scene->sostenuto, pressed);
...
static void handle_switch_event(const event_t* event, void* context) {
  ...
  scene_notify_hold_for(&scene->expr_switch, pressed);
