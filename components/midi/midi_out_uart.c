...
static bool s_repl_override = false;
...
void midi_out_uart_set_mode_override(midi_transmit_mode_t mode) {
  s_current_mode = mode;
  s_repl_override = true;
  ESP_LOGI(TAG, "UART transmit mode override: %d", mode);
}

bool midi_out_uart_is_override_active(void) {
  return s_repl_override;
}

void midi_out_uart_clear_override(void) {
  s_repl_override = false;
  ESP_LOGD(TAG, "UART transmit mode override cleared");
}
