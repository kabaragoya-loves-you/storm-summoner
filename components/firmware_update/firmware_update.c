...
esp_err_t assets_update_start(const uint8_t *data, size_t len) {
  ...
  // Find the `assets` partition. With the split layout this is STRICTLY the
  // read-only shared content (UI images + shared MIDI device DB). User data
  // (scenes, user-defined / cloned pedals, parsed-device cache) lives on the
  // separate `userdata` partition and is NEVER touched by an assets OTA.
