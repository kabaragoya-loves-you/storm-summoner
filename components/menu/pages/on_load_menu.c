...
#define TAG "MENU_ON_LOAD"

// Forward declarations
lv_obj_t* menu_page_on_load_scene_create(void);
...
static menu_item_t s_on_load_items[MAX_ON_LOAD_ACTIONS];
static char s_slot_labels[LABEL_BUFFER_SETS][MAX_ON_LOAD_ACTIONS][48];

// Action config contexts for each slot
static action_config_context_t s_slot_action_ctx[MAX_ON_LOAD_ACTIONS];

// Which slot we're currently editing
static uint8_t s_editing_slot = 0;

// Temp actions for slots that don't exist in scene yet
static action_t s_temp_actions[MAX_ON_LOAD_ACTIONS];
static bool s_using_temp[MAX_ON_LOAD_ACTIONS];
...
static void on_action_complete(action_config_context_t* ctx, action_t* action) {
  ...
  if (s_using_temp[slot] && action && action->type != ACTION_NONE) {
    // Copy the temp action to the scene's on_load array
    scene_t* scene = scene_get_current();
    if (scene && slot < MAX_ON_LOAD_ACTIONS) {
      memcpy(&scene->on_load[slot], action, sizeof(action_t));
      // Update count if this slot extends the array
      if (slot >= scene->num_on_load_actions) {
        scene->num_on_load_actions = slot + 1;
      }
      ESP_LOGI(TAG, "Added on-load action %d to scene", slot + 1);
    }
...
static void nav_to_slot(void* user_data) {
  ...
  if (slot >= MAX_ON_LOAD_ACTIONS) return;
  
  s_editing_slot = slot;
  s_using_temp[slot] = false;
  
  uint8_t scene_index = scene_get_current_index();
  action_t* action = scene_get_on_load_action(scene_index, slot);
  
  if (!action) {
    // Initialize temp action as NONE
  ...
  s_slot_action_ctx[slot].source_title = "On-Load";
  
  static char slot_titles[MAX_ON_LOAD_ACTIONS][24];
  snprintf(slot_titles[slot], sizeof(slot_titles[slot]), "Load Action %d", slot + 1);
  s_slot_action_ctx[slot].detail_title = slot_titles[slot];
  
  s_slot_action_ctx[slot].return_page = menu_page_on_load_scene_create;
...
lv_obj_t* menu_page_on_load_scene_create(void) {
  scene_t* scene = scene_get_current();
  if (!scene) {
    return menu_create_page_2line("On-Load", NULL, 0);
  }
  
  ESP_LOGI(TAG, "Creating On-Load page");
  
  int buf = get_next_buffer_set();
  uint8_t scene_index = scene_get_current_index();
  
  // Always show all 4 slots
  for (int i = 0; i < MAX_ON_LOAD_ACTIONS; i++) {
    action_t* action = scene_get_on_load_action(scene_index, i);
    char action_name[32];
    ...
    snprintf(s_slot_labels[buf][i], sizeof(s_slot_labels[buf][i]),
      "Load Action %d\n%s", i + 1, action_name);
    s_on_load_items[i] = (menu_item_t){
      s_slot_labels[buf][i], nav_to_slot, (void*)(uintptr_t)i, true
    };
  }
  
  return menu_create_page_2line("On-Load", s_on_load_items, MAX_ON_LOAD_ACTIONS);
}
