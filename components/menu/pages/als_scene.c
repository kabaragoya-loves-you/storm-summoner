...
#define TAG "MENU_ALS_SCENE"

// Forward declarations
lv_obj_t* menu_page_als_scene_create(void);
...
static void start_mode_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  scene->als.start_mode = (selected_index < 3) ? modes[selected_index]
...
static void cc_slot_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  if (selected_index == 0) {
    scene->als.cc_numbers[slot] = 0;
  } else if (selected_index < s_cc_options.count) {
    scene->als.cc_numbers[slot] = s_cc_options.cc_numbers[selected_index];
  }
  
  // Recalculate num_cc_numbers
  scene->als.num_cc_numbers = 0;
  for (int i = 0; i < 4; i++) {
    if (scene->als.cc_numbers[i] > 0) {
      scene->als.num_cc_numbers++;
    }
  }
...
static void polarity_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  if (selected_index < 3) {
    scene->als.polarity = polarities[selected_index];
    persist_scene_changes();
    ESP_LOGI(TAG, "ALS polarity set to: %s", polarity_to_string(scene->als.polarity));
  }
...
static void curve_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  if (selected_index < 4) {
    scene->als.curve.type = curves[selected_index];
    persist_scene_changes();
    ESP_LOGI(TAG, "ALS curve set to: %s", curve_type_to_string(scene->als.curve.type));
  }
...
// ============================================================================
// Base Note Roller (for Notes mode)
// ============================================================================
...
static void base_note_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  scene->als.base_note = (uint8_t)selected_index;
  persist_scene_changes();
  
  char note_name[8];
  get_note_name(scene->als.base_note, note_name, sizeof(note_name));
  ESP_LOGI(TAG, "ALS base note set to: %s (%d)", note_name, scene->als.base_note);
...
static lv_obj_t* base_note_roller_create(void) {
  ...
  uint32_t current = scene->als.base_note;
...
static void range_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  scene->als.note_range = (uint8_t)((selected_index + 1) * 12);
  persist_scene_changes();
  
  ESP_LOGI(TAG, "ALS note range set to: %d octaves (%d semitones)", 
    selected_index + 1, scene->als.note_range);
...
static lv_obj_t* range_roller_create(void) {
  ...
  uint32_t current = (scene->als.note_range / 12);
...
static void velocity_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  scene->als.velocity = (uint8_t)(selected_index + 1);
  persist_scene_changes();
  
  ESP_LOGI(TAG, "ALS velocity set to: %d", scene->als.velocity);
...
static lv_obj_t* velocity_roller_create(void) {
  ...
  uint8_t vel = scene->als.velocity;
  if (vel == 0) vel = 100;
...
static void nudge_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  if (pct > 100) pct = 100;
  scene_set_als_tempo_nudge_pct(scene_get_current_index(), pct);
...
static lv_obj_t* nudge_roller_create(void) {
  ...
  uint8_t cur = scene_get_als_tempo_nudge_pct(scene_get_current_index());
  if (cur > 100) cur = 100;
  uint32_t idx = cur / 5;
...
lv_obj_t* menu_page_als_scene_create(void) {
  ...
  snprintf(s_start_mode_label[buf], sizeof(s_start_mode_label[buf]),
    "Start Mode\n%s", start_mode_display(scene->als.start_mode));
  s_als_items[item_count++] = (menu_item_t){
  ...
  if (scene->als.output_type == OUTPUT_TYPE_CC) {
    // CC slots
    for (int i = 0; i < 4; i++) {
      uint8_t cc_num = scene->als.cc_numbers[i];
      if (cc_num > 0) {
