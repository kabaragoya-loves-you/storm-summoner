...
#include "scene.h"
#include "note_track_mode_mapping.h"
#include "curve.h"
...
static void mode_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  const note_track_mode_mapping_t* mapping = note_track_get_mode_mapping((uint8_t)selected_index);
  if (!mapping) {
  ...
  if (scene) {
    scene->note_track.enabled = mapping->enabled;
    scene->note_track.output_type = mapping->output_type;
    persist_scene_changes();
    ESP_LOGI(TAG, "Note Track mode set to: %s", mapping->display_name);
  }
...
static lv_obj_t* mode_roller_create(void) {
  ...
  options[0] = '\0';
  for (uint8_t i = 0; i < NUM_NOTE_TRACK_USER_MODES; i++) {
    if (i > 0) strcat(options, "\n");
    strcat(options, note_track_get_mode_name(i));
  }

  uint32_t current_idx = note_track_get_current_mode_index(scene_get_current());
...
static lv_obj_t* cc_roller_create(void) {
  ...
  return menu_create_roller_page("Parameter", s_cc_options.options_str, cur, cc_confirm_cb, NULL);
}
...
lv_obj_t* menu_page_note_track_scene_create(void) {
  ...
  uint8_t mode_idx = note_track_get_current_mode_index(scene);
  snprintf(s_mode_label[buf], sizeof(s_mode_label[buf]), "Mode\n%s",
    note_track_get_mode_name(mode_idx));
  ...
  if (scene->note_track.output_type == OUTPUT_TYPE_CC) {
    ...
    if (cc_num > 0) {
      ...
      if (cc_name && strcmp(cc_name, "Undefined") != 0)
        snprintf(s_cc_label[buf], sizeof(s_cc_label[buf]), "Parameter\n%s", cc_name);
      else
    ...
    s_nt_items[n++] = (menu_item_t){
      s_cc_label[buf], nav_to_cc, NULL, true, MENU_ITEM_KIND_ROLLER
    };
  }
  ...
  if (scene->note_track.output_type == OUTPUT_TYPE_LFO_RATE ||
    ...
    s_nt_items[n++] = (menu_item_t){
      s_lfo_target_label[buf], nav_to_lfo_target, NULL, true, MENU_ITEM_KIND_ROLLER
    };
  }
