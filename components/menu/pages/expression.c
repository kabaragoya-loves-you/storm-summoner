...
static void start_mode_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  scene->expression.start_mode = (selected_index < 3) ? modes[selected_index]
...
static lv_obj_t* start_mode_roller_create(void) {
  ...
  switch (scene->expression.start_mode) {
...
static void nudge_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  if (pct > 100) pct = 100;
  scene_set_expression_tempo_nudge_pct(scene_get_current_index(), pct);
...
static lv_obj_t* nudge_roller_create(void) {
  ...
  uint8_t cur = scene_get_expression_tempo_nudge_pct(scene_get_current_index());
  if (cur > 100) cur = 100;
  uint32_t idx = cur / 5;
  ...
  switch (mode) {
    case EXPRESSION_MODE_PEDAL: {
      snprintf(s_start_mode_label[buf], sizeof(s_start_mode_label[buf]),
        "Start Mode\n%s", start_mode_display(scene->expression.start_mode));
      s_expr_items[item_count++] = (menu_item_t){
