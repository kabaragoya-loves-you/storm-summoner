...
static void output_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  output_type_t new_type = (selected_index == 0) ? OUTPUT_TYPE_CC : OUTPUT_TYPE_NOTE;
  scene->proximity.output_type = new_type;
  
  // Note: proximity_mode is no longer used - handler checks scene->proximity.output_type
  
  persist_scene_changes();
  
  ESP_LOGI(TAG, "Proximity output set to: %s", 
    new_type == OUTPUT_TYPE_CC ? "CC" : "Notes (Theremin)");
...
static lv_obj_t* output_roller_create(void) {
  ...
  uint32_t current = (scene->proximity.output_type == OUTPUT_TYPE_CC) ? 0 : 1;
  return menu_create_roller_page("Output", "Control Change\nNotes (Theremin)", current, output_confirm_cb, NULL);
}
...
lv_obj_t* menu_page_proximity_scene_create(void) {
  ...
  const char* output_name = (scene->proximity.output_type == OUTPUT_TYPE_CC) ? 
    "Control Change" : "Notes (Theremin)";
