...
static void rate_mode_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  if (selected_index < 7) {
    scene->lfo1_config.rate_mode = modes[selected_index];
    lfo_apply_config(0, &scene->lfo1_config);
    persist_scene_changes();
  }
...
static lv_obj_t* rate_mode_roller_create(void) {
  ...
  uint32_t current = 0;
  switch (scene->lfo1_config.rate_mode) {
...
static void output_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  scene->lfo1.output_type = (selected_index == 0) ? OUTPUT_TYPE_CC : OUTPUT_TYPE_NOTE;
  persist_scene_changes();
...
static lv_obj_t* output_roller_create(void) {
  ...
  uint32_t current = (scene->lfo1.output_type == OUTPUT_TYPE_CC) ? 0 : 1;
...
lv_obj_t* menu_page_lfo1_scene_create(void) {
  ...
  const char* output_name = (scene->lfo1.output_type == OUTPUT_TYPE_CC) ? 
