...
static void enabled_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  scene->lfo1_config.enabled = (selected_index == 1);
  scene->lfo1.enabled = scene->lfo1_config.enabled;
  ...
  ESP_LOGI(TAG, "LFO1 %s", scene->lfo1_config.enabled ? "enabled" : "disabled");
...
static lv_obj_t* enabled_roller_create(void) {
  ...
  uint32_t current = scene->lfo1_config.enabled ? 1 : 0;
  return menu_create_roller_page("LFO1", "Disabled\nEnabled", current, enabled_confirm_cb, NULL);
}

static void nav_to_enabled(void* user_data) {
  (void)user_data;
  menu_navigate_to("LFO1", enabled_roller_create);
}
...
static lv_obj_t* rate_mode_roller_create(void) {
  ...
  for (int i = 0; i < s_rate_mode_count; i++) {
    if (s_rate_modes[i] == scene->lfo1_config.rate_mode) {
      current = (uint32_t)i;
      break;
    }
  }
...
static void output_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  // 0=CC, 1=Note, 2=LFO2 Rate, 3=LFO2 Depth, 4=RTG Rate, 5=S+H Rate, 6=Pitch Bend
  switch (selected_index) {
    case 0: scene->lfo1.output_type = OUTPUT_TYPE_CC; break;
    case 1: scene->lfo1.output_type = OUTPUT_TYPE_NOTE; break;
    case 2: scene->lfo1.output_type = OUTPUT_TYPE_LFO2_RATE; break;
    case 3: scene->lfo1.output_type = OUTPUT_TYPE_LFO2_DEPTH; break;
    case 4: scene->lfo1.output_type = OUTPUT_TYPE_RTG_RATE; break;
    case 5: scene->lfo1.output_type = OUTPUT_TYPE_SH_RATE; break;
    case 6: scene->lfo1.output_type = OUTPUT_TYPE_PITCH_BEND; break;
    default: scene->lfo1.output_type = OUTPUT_TYPE_CC; break;
  }
...
static lv_obj_t* output_roller_create(void) {
  ...
  uint32_t current = 0;
  switch (scene->lfo1.output_type) {
    case OUTPUT_TYPE_CC: current = 0; break;
    case OUTPUT_TYPE_NOTE: current = 1; break;
    case OUTPUT_TYPE_LFO2_RATE: current = 2; break;
    case OUTPUT_TYPE_LFO2_DEPTH: current = 3; break;
...
static void polarity_confirm_cb(uint32_t selected_index, void* user_data) {
  ...
  if (selected_index < 2) {
    scene->lfo1.polarity = polarities[selected_index];
    persist_scene_changes();
  }
...
static lv_obj_t* polarity_roller_create(void) {
  ...
  uint32_t current = (scene->lfo1.polarity == POLARITY_INVERTED) ? 1 : 0;
...
lv_obj_t* menu_page_lfo1_scene_create(void) {
  ...
  switch (scene->lfo1.output_type) {
    ...
    case OUTPUT_TYPE_LFO2_RATE: output_name = "LFO2 Rate"; break;
    case OUTPUT_TYPE_LFO2_DEPTH: output_name = "LFO2 Depth"; break;
