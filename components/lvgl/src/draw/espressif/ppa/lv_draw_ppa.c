...
void lv_draw_ppa_init(void)
{
    ...
    if(res != ESP_OK) LV_LOG_ERROR("PPA: Failed to register SRM client: %d", res);
    ...
    if(res != ESP_OK) LV_LOG_ERROR("PPA: Failed to register Fill client: %d", res);
    LV_ASSERT(res == ESP_OK);
    ...
    if(res != ESP_OK) LV_LOG_ERROR("PPA: Failed to register Blend client: %d", res);
    LV_ASSERT(res == ESP_OK);
...
static bool ppa_isr(ppa_client_handle_t ppa_client, ppa_event_data_t * event_data, void * user_data)
{
    ...
    // PPA operation completed - ensure writes are visible to CPU
    if(u->task_act) {
        lv_draw_task_t * t = u->task_act;
        lv_layer_t * layer = t->target_layer;
        lv_draw_buf_t * buf = layer->draw_buf;
        
        // Invalidate cache so CPU sees PPA-modified data
        esp_cache_msync(buf->data, buf->data_size, ESP_CACHE_MSYNC_FLAG_DIR_M2C | ESP_CACHE_MSYNC_FLAG_INVALIDATE);
    }
